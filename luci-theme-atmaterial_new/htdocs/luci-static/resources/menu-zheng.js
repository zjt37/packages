	...
	handleMenuExpand(ev) {
		...
		document.querySelectorAll('li.slide.active').forEach(function(li) {
			if (li !== ul1) {
				...
				const submenu = li.querySelector('.slide-menu');
		...
		if (ul2.style.display === '' || ul2.style.display === 'none') {
			ul2.style.display = 'block'; // 显示二级菜单
		} else {
			 ul2.style.display = 'none'; // 隐藏二级菜单
		}
        ...
        // 为每个菜单项添加图标
			...
			switch (child.title) {
					...
					case 'Bandwidth Monitor':
					     iconClass = 'bandwidth-icon';
					     break;
					case 'Logout':
					     iconClass = 'logout-icon';
					     break;
					case 'Reboot':
					     iconClass = 'reboot-icon';
					     break;
					default:
					     iconClass = '';
			}
			...
			ul.appendChild(E('li', { 'class': (hasChildren ? 'slide' + (isActive ? ' active' : '') : (isActive ? ' active' : ''))}, [
                ...
                // 在菜单项之前插入图标
					l === 1 ? E('span', { 'class': `menu-icon ${iconClass}` }) : '',
					_(child.title)  // 菜单名称
				]),
				submenu
			]));
