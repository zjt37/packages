...
. /usr/share/nodepool/utils.sh
GLOBAL_ACL_PATH=${TMP_ACL_PATH}/default
LUA_UTIL_PATH=/usr/lib/lua/luci/nodepool
...
run_redir() {
	...
	TCP)
		...
		tcp_node_socks_port=$(get_new_port $(config_t_get global tcp_node_socks_port 1170))
	...
	[ "${DNS_SHUNT}" = "smartdns" ] && {
		if command -v smartdns > /dev/null 2>&1; then
			...
			lua $APP_PATH/helper_smartdns_add.lua -FLAG "default" -SMARTDNS_CONF "/tmp/etc/smartdns/$CONFIG.conf" \
				...
				-LOCAL_GROUP ${group_domestic:-null} -REMOTE_GROUP "nodepool_proxy" -REMOTE_PROXY_SERVER ${TCP_SOCKS_server} -USE_DEFAULT_DNS "${USE_DEFAULT_DNS:-direct}" \
...
stop() {
	...
	busybox pgrep -af "${CONFIG}/" | awk '! /app\.sh|subscribe\.lua|rule_update\.lua|tasks\.sh|ujail/{print $1}' | xargs -r kill -9 >/dev/null 2>&1
