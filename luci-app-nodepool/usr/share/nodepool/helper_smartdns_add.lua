...
local RULES_PATH = "/usr/share/nodepool/rules"
...
if not REMOTE_GROUP or REMOTE_GROUP == "nil" then
	REMOTE_GROUP = "nodepool_proxy"
	if REMOTE_DNS then
		REMOTE_DNS = REMOTE_DNS:gsub("#", ":")
	end
	sys.call('sed -i "/nodepool/d" /etc/smartdns/custom.conf >/dev/null 2>&1')
end

local force_https_soa = api.uci_get_c("@global[0]", "force_https_soa") or 0
local proxy_server_name = "nodepool-proxy-server"
config_lines = {
...
local setflag = (NFTFLAG == "1") and "inet#nodepool#" or ""
...
if USE_BLOCK_LIST == "1" and is_file_nonzero(file_block_host) then
	local domain_set_name = "nodepool-block"
	tmp_lines = {
...
if is_file_nonzero(file_vpslist) then
	local domain_set_name = "nodepool-vpslist"
	tmp_lines = {
...
if USE_DIRECT_LIST == "1" and is_file_nonzero(file_direct_host) then
	local domain_set_name = "nodepool-directlist"
	tmp_lines = {
...
if USE_PROXY_LIST == "1" and is_file_nonzero(file_proxy_host) then
	local domain_set_name = "nodepool-proxylist"
	tmp_lines = {
...
if USE_GFW_LIST == "1" and is_file_nonzero(RULES_PATH .. "/gfwlist") then
	local domain_set_name = "nodepool-gfwlist"
	tmp_lines = {
...
if CHN_LIST ~= "0" and is_file_nonzero(RULES_PATH .. "/chnlist") then
	local domain_set_name = "nodepool-chnlist"
	tmp_lines = {
...
if IS_SHUNT_NODE then
	...
	if is_file_nonzero(file_white_host) then
		local domain_set_name = "nodepool-whitehost"
		tmp_lines = {
	...
	if is_file_nonzero(file_shunt_host) then
		local domain_set_name = "nodepool-shuntlist"
		tmp_lines = {
...
sys.call(string.format('echo "conf-file %s" >> /etc/smartdns/custom.conf', string.gsub(SMARTDNS_CONF, "nodepool", "nodepool*")))
