...
FWI=$(uci -q get firewall.nodepool.path 2>/dev/null)
...
add_script_mwan3() {
	del_script_mwan3
	[ -s "/etc/init.d/mwan3" ] && {
		sed -i '/start_service()/,/}/ s/^}/    \/usr\/share\/nodepool\/nftables.sh mwan3_start\n}/' /etc/init.d/mwan3
		sed -i '/stop_service().*{/a \    \/usr\/share\/nodepool\/nftables.sh mwan3_stop' /etc/init.d/mwan3
	}
}
