...
lua_api() {
	...
	echo $(lua -e "local api = require 'luci.nodepool.api' print(api.${func})")
}
...
ln_run() {
	...
	if [ -z "$persist_log_path" ] && [ "$sys_log" != "1" ]; then
		...
		if [ -n "${persist_log_path}" ]; then
			...
			local log_file=${persist_log_path}/nodepool_${protocol}_${ln_name}_$(date '+%F').log
		...
		if [ "${sys_log}" = "1" ]; then
			...
			${file_func:-echolog " - ${ln_name}"} "$@" 2>&1 | logger -t NODEPOOL_${protocol}_${ln_name} &
		fi
	fi
