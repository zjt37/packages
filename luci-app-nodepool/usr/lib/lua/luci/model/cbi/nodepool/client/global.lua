api = require "luci.nodepool.api"
datatypes = api.datatypes
local fs = api.fs
has_singbox = api.finded_com("sing-box")
has_xray = api.finded_com("xray")
local has_gfwlist = fs.access("/usr/share/nodepool/rules/gfwlist")
local has_chnlist = fs.access("/usr/share/nodepool/rules/chnlist")
local has_chnroute = fs.access("/usr/share/nodepool/rules/chnroute")
...
if (has_singbox or has_xray) and #nodes_table > 0 then
	if #normal_list > 0 or #iface_list > 0 then
		if current_node.protocol == "_shunt" then
			local shunt_lua = loadfile("/usr/lib/lua/luci/model/cbi/nodepool/client/include/shunt_options.lua")
