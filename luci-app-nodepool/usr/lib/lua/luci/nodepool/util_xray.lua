module("luci.nodepool.util_xray", package.seeall)
...
local function get_domain_excluded()
	local path = "/usr/share/nodepool/domains_excluded"
		...
		if node.type ~= "Xray" then
			if node.type == "Socks" then
				...
				if run_socks_instance then
					sys.call(string.format('/usr/share/nodepool/app.sh run_socks "%s"> /dev/null',
...
function gen_config_server(node)
	...
	if node.outbound_node then
		...
		if node.outbound_node == "_iface" and node.outbound_node_iface then
			...
			outbound = require("luci.nodepool.util_xray").gen_outbound(nil, outbound_node_t, "outbound")
		end
		if outbound then
	...
	local config = {
		log = {
			-- error = "/tmp/etc/nodepool_server/log/" .. user[".name"] .. ".log",
...
function gen_config(var)
	...
	if node_id then
		...
		if node and node.protocol == "_shunt" then
			...
			local function foreach_shunt_rule(callback)
				...
				local domain_list = read_proxy_list("/usr/share/nodepool/rules/proxy_host")
				local ip_list = read_proxy_list("/usr/share/nodepool/rules/proxy_ip")
	...
	if (remote_dns_udp_server and remote_dns_udp_port) or (remote_dns_tcp_server and remote_dns_tcp_port) or remote_dns_doh or #node_dns > 0 then
		...
		if dns_listen_port and next(_direct_dns) then
			local domain = {}
			local nodes_domain_text = sys.exec([[uci show nodepool | sed -n "s/.*\.\(address\|download_address\)='\([^']*\)'/\2/p" | sort -u]])
	...
	if inbounds or outbounds then
		local config = {
			...
			log = {
				-- error = string.format("/tmp/etc/nodepool/%s.log", node[".name"]),
