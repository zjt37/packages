...
function to_move(app_name,file)
	...
	local flag = sys.call('busybox pgrep -af "nodepool/.*'.. app_name ..'" >/dev/null')
	if flag == 0 then
		sys.call("/etc/init.d/nodepool stop")
	end
	...
	sys.call(cmd_rm_tmp)
	if flag == 0 then
		sys.call("/etc/init.d/nodepool restart >/dev/null 2>&1 &")
	end
...
function get_version()
	local version = sys.exec("opkg list-installed luci-app-nodepool 2>/dev/null | awk '{print $3}'")
...
function to_check_self()
	local url = "https://raw.githubusercontent.com/Openwrt-Passwall/openwrt-nodepool/main/luci-app-nodepool/Makefile"
	local tmp_file = "/tmp/nodepool_makefile"
	local gh_proxy = uci_get_c("@global_app[0]", "github_proxy") or "0"
	local return_code, result
	if gh_proxy == "1" then
		url = "https://gh-proxy.org/" .. url
	...
	local remote_version = sys.exec("echo -n $(grep '^PKG_VERSION' /tmp/nodepool_makefile | head -n 1 | awk -F '=' '{print $2}')")
	exec("/bin/rm", {"-f", tmp_file})
...
function set_default_cbi()
	...
	if true then
		...
		function Map.__init__(self, config, ...)
			...
			self.api = require "luci.nodepool.api"
		end
