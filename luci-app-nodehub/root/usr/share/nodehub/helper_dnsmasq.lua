...
function copy_instance(var)
	...
	for line in io.lines("/tmp/etc/dnsmasq.conf." .. DEFAULT_DNSMASQ_CFGID) do
		local filter
		if line:find("nodehub") then filter = true end
	...
	if TMP_DNSMASQ_PATH then
		sys.call("rm -rf " .. TMP_DNSMASQ_PATH .. "/*nodehub*")
	end
...
function add_rule(var)
	...
	local cache_text = ""
	local nodes_address_md5 = sys.exec("echo -n $(uci show nodehub | grep '\\.address') | md5sum")
	local new_rules = sys.exec("echo -n $(find /usr/share/nodehub/rules -type f | xargs md5sum)")
	...
	local setflag_4= (NFTFLAG == "1") and "4#inet#nodehub#" or ""
	local setflag_6= (NFTFLAG == "1") and "6#inet#nodehub#" or ""
	...
	if not fs.access(CACHE_DNS_PATH) then
		...
		if USE_CHINADNS_NG == "0" and USE_BLOCK_LIST == "1" then
			local geosite_arg = ""
			local f = io.open("/usr/share/nodehub/rules/block_host")
			if f then
		...
		if USE_DIRECT_LIST == "1" then
			...
			if fwd_dns then
				...
				--始终用国内DNS解析直连（白名单）列表
				local geosite_arg = ""
				local f = io.open("/usr/share/nodehub/rules/direct_host")
				if f then
					for line in f:lines() do
						if not line:find("#") and line:find("geosite:") then
							line = string.match(line, ":([^:]+)$")
		...
		if USE_PROXY_LIST == "1" then
			...
			if fwd_dns then
				...
				local geosite_arg = ""
				local f = io.open("/usr/share/nodehub/rules/proxy_host")
				if f then
					for line in f:lines() do
						if not line:find("#") and line:find("geosite:") then
							line = string.match(line, ":([^:]+)$")
		...
		if USE_GFW_LIST == "1" then
			...
			if fwd_dns then
				...
				local f = io.open("/usr/share/nodehub/rules/gfwlist")
				if f then
					for line in f:lines() do
		...
		if CHN_LIST ~= "0" then
			...
			if fwd_dns then
				...
				local f = io.open("/usr/share/nodehub/rules/chnlist")
				if f then
					for line in f:lines() do
						if line ~= "" and not line:find("#") and not check_excluded_domain(line) then
	...
	if USE_CHINADNS_NG == "0" then
		log("  - NodeHub必须依赖于Dnsmasq，如果你自行配置了错误的DNS流程，将会导致域名(直连/代理域名)分流失效！！！")
	end
end
