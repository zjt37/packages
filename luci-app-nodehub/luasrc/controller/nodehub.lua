...
-- Copyright (C) 2026 Openwrt-Nodehub Organization

module("luci.controller.nodehub", package.seeall)
local api = require "luci.nodehub.api"
...
function index()
	if not nixio.fs.access("/etc/config/nodehub") then
		if nixio.fs.access("/usr/share/nodehub/0_default_config") then
			luci.sys.call('cp -f /usr/share/nodehub/0_default_config /etc/config/nodehub')
		else return end
	end
	local api = require "luci.nodehub.api"
	...
	e.dependent = true
	e.acl_depends = { "luci-app-nodehub" }
	--[[ Client ]]
...
function reset_config()
	uci:revert(c_config)
	luci.sys.call('/etc/init.d/nodehub stop')
	if luci.sys.call('[ -s "/usr/share/nodehub/0_default_config" ]') == 0 then
		luci.sys.call('cp -f /usr/share/nodehub/0_default_config /etc/config/nodehub')
		api.log(" * 恢复默认配置成功。")
	else
...
function link_add_node()
	...
	if chunk and chunk_index ~= nil and total_chunks ~= nil then
		...
		if chunk_index + 1 == total_chunks then
			luci.sys.call("lua /usr/share/nodehub/subscribe.lua add " .. group)
		end
	end
end
...
function gen_client_config()
	...
	luci.sys.call(string.format("/usr/share/nodehub/app.sh run_socks flag=config_%s node=%s bind=127.0.0.1 socks_port=1080 config_file=%s no_run=1", id, id, config_file))
...
function urltest_node()
	...
	local e = {}
	e.index = index
	local result = luci.sys.exec(string.format("/usr/share/nodehub/test.sh url_test_node %s %s", id, "urltest_node"))
	local code = tonumber(luci.sys.exec("echo -n '" .. result .. "' | awk -F ':' '{print $1}'") or "0")
	if code ~= 0 then
		local use_time_str = luci.sys.exec("echo -n '" .. result .. "' | awk -F ':' '{print $2}'")
		local use_time = tonumber(use_time_str)
...
function update_rules()
	...
	luci.sys.call("lua /usr/share/nodehub/rule_update.lua log '" .. update .. "' > /dev/null 2>&1 &")
	http_write_json()
end
...
function rollback_rules()
	...
	if geo2rule == "1" and rules ~= "" then
		luci.sys.call("lua /usr/share/nodehub/rule_update.lua log '" .. rules .. "' rollback > /dev/null")
	end
	http_write_json_ok()
end
...
local backup_files = {
    ...
    "/usr/share/nodehub/rules/direct_ip",
    "/usr/share/nodehub/rules/proxy_host",
    "/usr/share/nodehub/rules/proxy_ip"
}

function create_backup()
	local date = os.date("%y%m%d%H%M")
	local tar_file = "/tmp/nodehub-" .. date .. "-backup.tar.gz"
	fs.remove(tar_file)
	local cmd = "tar -czf " .. tar_file .. " " .. table.concat(backup_files, " ")
	luci.sys.call(cmd)
	http.header("Content-Disposition", "attachment; filename=nodehub-" .. date .. "-backup.tar.gz")
	http.header("X-Backup-Filename", "nodehub-" .. date .. "-backup.tar.gz")
...
function restore_backup()
	...
	local ok, err = pcall(function()
		...
		if chunk_index + 1 == total_chunks then
			uci:revert(c_config)
			api.log(" * NodeHub 配置文件上传成功…")
			local temp_dir = '/tmp/nodehub_bak'
