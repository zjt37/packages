...
local has_gfwlist = fs.access("/usr/share/nodehub/rules/gfwlist")
local has_chnlist = fs.access("/usr/share/nodehub/rules/chnlist")
local has_chnroute = fs.access("/usr/share/nodehub/rules/chnroute")
...
if (has_singbox or has_xray) and #nodes_table > 0 then
	if #normal_list > 0 or #iface_list > 0 then
		if current_node.protocol == "_shunt" then
			local shunt_lua = loadfile("/usr/lib/lua/luci/model/cbi/nodehub/client/include/shunt_options.lua")
