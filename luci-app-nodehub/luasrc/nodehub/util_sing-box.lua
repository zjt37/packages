module("luci.nodehub.util_sing-box", package.seeall)
...
function gen_config_server(node)
	...
	if node.outbound_node then
		...
		if node.outbound_node == "_iface" and node.outbound_node_iface then
			...
			outbound = require("luci.nodehub.util_sing-box").gen_outbound(nil, outbound_node_t, "outbound")
		end
		if outbound then
...
function gen_config(var)
	...
	if dns_listen_port then
		...
		if direct_dns_udp_server or direct_dns_tcp_server then
			local domain = {}
			local nodes_domain_text = sys.exec([[uci show nodehub | sed -n "s/.*\.address='\([^']*\)'/\1/p" | sort -u]])
