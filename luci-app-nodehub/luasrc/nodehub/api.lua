module("luci.nodehub.api", package.seeall)
appname = "nodehub"
c_config = "nodehub"
s_config = "nodehub_server"
local com = require "luci.nodehub.com"
nixio = require "nixio"
fs = require "nixio.fs"
...
function set_cache_var(key, val)
	sys.call(string.format('. /usr/share/nodehub/utils.sh ; set_cache_var %s "%s"', key, val))
end

function get_cache_var(key)
	local val = sys.exec(string.format('. /usr/share/nodehub/utils.sh ; echo -n $(get_cache_var %s)', key))
	if val == "" then val = nil end
	return val
end

function get_new_port()
	local cmd_format = ". /usr/share/nodehub/utils.sh ; echo -n $(get_new_port %s tcp,udp)"
...
function curl_auto(url, file, args)
	...
	if localhost_proxy == "1" then
		return curl_base(url, file, args) -- 当路由器本机开启代理时，采用nodehub规则进行访问
	else
		local return_code, result = curl_proxy(url, file, args)
		if not return_code or return_code ~= 0 then
...
function to_move(app_name,file)
	...
	local flag = sys.call('busybox pgrep -af "nodehub/.*'.. app_name ..'" >/dev/null')
	if flag == 0 then
		sys.call("/etc/init.d/nodehub stop")
	end
	...
	sys.call(cmd_rm_tmp)
	if flag == 0 then
		sys.call("/etc/init.d/nodehub restart >/dev/null 2>&1 &")
	end
...
function get_version()
	local version = sys.exec("opkg list-installed luci-app-nodehub 2>/dev/null | awk '{print $3}'")
	if not version or #version == 0 then
		version = sys.exec("apk list luci-app-nodehub 2>/dev/null | awk '/installed/ {print $1}' | cut -d'-' -f4-")
	end
	return (version or ""):gsub("\n", ""):match("^([^-]+)")
end

function to_check_self()
	local url = "https://raw.githubusercontent.com/Openwrt-Nodehub/openwrt-nodehub/main/luci-app-nodehub/Makefile"
	local tmp_file = "/tmp/nodehub_makefile"
	local gh_proxy = uci_get_c("@global_app[0]", "github_proxy") or "0"
	local return_code, result
	if gh_proxy == "1" then
		url = "https://gh-proxy.org/" .. url
	...
	local remote_version = sys.exec("echo -n $(grep '^PKG_VERSION' /tmp/nodehub_makefile | head -n 1 | awk -F '=' '{print $2}')")
	exec("/bin/rm", {"-f", tmp_file})
...
function set_default_cbi()
	...
	if true then
		...
		function Map.__init__(self, config, ...)
			...
			self.api = require "luci.nodehub.api"
		end
